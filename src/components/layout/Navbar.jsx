    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-border shadow-sm">
      <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV.map(({ href, label }) => (
            <Link key={href} href={href}
              className={clsx(
                'px-4 py-2 rounded text-sm font-sans font-medium transition-colors',
                pathname === href
                  ? 'text-violet-700 bg-violet-100'
                  : 'text-violet-800 hover:text-violet-700 hover:bg-violet-50'
              )}>
              {label}
            </Link>
          ))}
              ...
              <div className="relative" ref={ddRef}>
                ...
                {ddOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl border border-border shadow-card-lg py-2 z-50">
                    ...
                    <MenuLink href="/dashboard"  icon={LayoutDashboard} label="Dashboard"        onClick={() => setDdOpen(false)} />
                    <MenuLink href="/classes"    icon={Users}            label="My Classes"       onClick={() => setDdOpen(false)} />
                    {profile?.role === 'instructor' && (
                      <MenuLink href="/instructor" icon={GraduationCap} label="Instructor Portal" onClick={() => setDdOpen(false)} />
                    )}
                    {profile?.role === 'admin' && (
        ...
        <button
          ...
          className="md:hidden p-2 rounded text-violet-700 hover:bg-violet-100 transition-colors"
        >
        ...
        <div className="md:hidden border-t border-border bg-white px-5 py-4 space-y-1">
          {NAV.map(({ href, label }) => (
            <Link key={href} href={href}
              ...
              className="block py-2.5 text-sm font-medium text-violet-800 border-b border-border/60">
              {label}
            </Link>
          ))}
          <div className="pt-3 flex flex-col gap-2">
            {user ? (
              <>
                <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="btn-outline w-full">Dashboard</Link>
                <button
                  onClick={async () => { await signOut(); router.push('/'); setMobileOpen(false) }}
                  className="btn-ghost text-red-500 w-full"
                >
                  Sign out
                </button>
              </>
              ...
              <>
                <Link href="/auth/login"    onClick={() => setMobileOpen(false)} className="btn-outline w-full">Sign in</Link>
                <Link href="/auth/register" onClick={() => setMobileOpen(false)} className="btn-primary w-full">Get started free</Link>
              </>
            )}
          </div>
        </div>
